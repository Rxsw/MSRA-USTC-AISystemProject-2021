...
[代码\\hpo\\main.py](代码/hpo/main.py)

[代码\\hpo\\utils.py](代码/hpo/utils.py)

**手动调参**

说明：

由于缺少实践经验，不了解各个超参数之间的组合关系（比如某种超参数组合可能比单纯的各参数最优的组合的表现更优），因此初步想法是采用**控制变量法**，**每次只改变一个超参数**，找到每个超参数的使得单独表现最优的值，最后再将这些单独最优值组合起来，试图找到最好的超参数组合。
...
由于个人机器性能限制，**本次NNI自动调参进行了三次，一次本地，两次远程。**

第一次是在自己的机器上运行（**由于性能限制，调小了epochs以完成足够多的trials**）

**有WebUI结果，有输出的结果文件**，**共10个trials，运行3h 18m 44s，使用一张Gpu**
...
**第一次：本地  来源：输出结果文件FcMxgqBE\\db\\** **nni.sqlite**
...
**第二次：Bitahub nniv1.8 来源：结果文件cXbLe89m \\db\\** **nni.sqlite**
...
**第三次：Bitahub nniv1.8 来源：结果文件xq4PrepE \\db\\** **nni.sqlite**
...
可以看到，NNI的最终最优结果比人工调参的结果稍低，但相信如果继续实验，进行更多的trials一定能很快找到更好的解。与此同时，为达到这样的效果，手动调参进行了大量的工作，而NNI自动调参只是修改了两次搜索空间而已。NNI的高效性在这里体现的非常充分。
...
![](media/3cf6216e9e6d1ee5e4b448ecf6608502.png)

![](media/5d4ccfa98165b3b6b80ed73591b5cf65.png)

![](media/b85790ced686c6439694832647105380.png)![](media/939a3fb91fbc96059bc4970b52029ba8.png)

更多截图请看**结果/截图** 文件夹里查看。

**4.实验总结**

在本次实验中，遇到了许多问题，也积累了很多经验，都值得一说
...
在单个gpu上并行的跑多个trials并不会有效率提升，效率反而会下降。
...
![](media/2828954c0f1c1a99520e629c97ee55f3.png)![](media/d9b29c6ed59e45cc64fc1944a52f36a2.png)WebUI的使用非常方便，不仅能随时随地的图形化的查看实验情况，还能临时更改实验的上限时间和trial数，甚至即使整个实验完成，只要未彻底停止实验，也能通过WebUI追加trials以继续运行。还可以详细的看到每一个trial的运行情况，结果曲线。

![](media/0e0da3f2d26f970e05581ca4d8dc39bf.png)
